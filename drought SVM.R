...
length_classifier = ksvm( outcome ~ ., data = train_r, kernel = "rbfdot", C = 50)
length_classifier
length_predictions = predict(length_classifier, test_r)

#model analysis
test_r_label = test_r$outcome
results = HMeasure(test_r_label, length_predictions)
plotROC(results) #ROC curve
summary(results) #view AU
